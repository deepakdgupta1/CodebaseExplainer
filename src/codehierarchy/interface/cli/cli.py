    ...
    try:
        ...
        if results:
            ...
            generator.generate_documentation(
                results['graph'], results['summaries'])
            print(
                f"\n✅ Analysis complete! Output in {
...
@click.option('--index-dir', type=click.Path(exists=True),
              required=True, help='Path to index directory')
@click.option('--mode',
              type=click.Choice(['keyword',
                                 'semantic',
                                 'hybrid']),
              default='hybrid')
