...
def compute_cyclomatic_complexity(node: Node, language: str) -> int:
    ...
    def traverse(n: Node) -> None:
        ...
        if n.type in branching_types:
            ...
            if n.type == 'binary_expression' or (language == 'python' and n.type == 'boolean_operator'):
                # In Python boolean_operator is 'and'/'or'
                # In TS binary_expression can be +, -, etc. Need to check operator child.
                pass # Simplified: count all for now or refine
                # Refinement for TS:
                if language in ['typescript', 'tsx', 'javascript'] and n.type == 'binary_expression':
                     # Check operator
