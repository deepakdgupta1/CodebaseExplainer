...
class ModelManager:
    ...
    def load_model(self) -> bool:
        ...
        if not self.lms_path:
            logging.warning(
                "LM Studio CLI 'lms' not found. Please ensure the model is loaded manually in LM Studio.")
            logging.info(f"Expected Model: {self.config.model_name}")
            logging.info(
                f"Settings: Context={
                    self.config.context_window}, GPU Offload={
                    self.config.gpu_offload_ratio}")
            return False
        ...
        try:
            ...
            # Note: The exact flags for lms might vary, this is a best-effort
            # implementation based on common CLI patterns.
            cmd = [
