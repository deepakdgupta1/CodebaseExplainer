...
class SystemConfig(BaseModel):
    ...
    temp_dir: Path = Field(
        default=Path(".codehierarchy/temp"),
    ...
    checkpointing_enabled: bool = Field(
        default=True,
        description="Enable checkpointing for long processes"
    )
...
class ParsingConfig(BaseModel):
    ...
    max_file_size_mb: float = Field(
        default=10.0,
        description="Max file size to process in MB"
    )
...
class GraphConfig(BaseModel):
    storage_mode: Literal["in_memory", "disk"] = Field(
        default="in_memory",
        description="Graph storage mode"
    )
    cache_size_gb: float = Field(
        default=2.0,
        description="Max size for source code cache"
    )
    compute_metrics: bool = Field(
        default=True,
        description="Compute centrality and other metrics"
    )
...
class LLMConfig(BaseModel):
    ...
    batch_size: int = Field(
        default=10,
        description="Number of nodes per LLM call"
    )
    ...
    max_retries: int = Field(
        default=2,
        description="Max retries for failed calls"
    )

    # Advanced LM Studio Settings
    context_overflow_policy: Literal["stopAtLimit", "rollingWindow"] = Field(
        default="stopAtLimit",
        description="Context overflow policy"
    )
    top_k: int = Field(
        default=40,
    ...
    gpu_offload_ratio: float = Field(
        default=1.0,
        description="GPU Offload ratio (0.0 to 1.0)"
    )
    cpu_threads: int = Field(
        default=4,
        description="CPU Thread Pool Size"
    )
    ...
    flash_attention: bool = Field(
        default=False,
        description="Use Flash Attention (KV Cache Offload)"
    )
    use_mmap: bool = Field(
        default=True,
...
class SearchConfig(BaseModel):
    ...
    nprobe: int = Field(
        default=64,
        description="FAISS nprobe parameter"
    )
    rrf_k: int = Field(
        default=60,
